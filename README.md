...
- **安装流程**：
  ...
  3. 安装 `git-delta`（deb 包）
   4. 以 `node` 用户安装：pnpm、uv（OpenCode 通过容器的 `postCreateCommand` 以 pnpm 安装，享受 pnpm store 缓存）
  5. 配置 zsh + powerlevel10k 主题
   6. 复制 `init-firewall.sh` → `/usr/local/bin/`（配置 sudoers 白名单），`init-ssh.sh` 和 `start-opencode-web.py` → `~/.local/bin/`
   7. 创建 OpenCode 数据和缓存目录（`~/.local/share/opencode`、`~/.config/opencode`、`~/.cache/opencode`）
   8. 设置 git 全局配置（`user.name "opencode"`、`user.email "dev@opencode.ai"`）
